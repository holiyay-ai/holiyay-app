...
import type {
	...
	ShareInfo,
	ShareLink,
} from "@holiyay/api/types"

// API base URL configuration
function getBaseUrl(): string {
	// Server-side: use internal URL in Docker, localhost otherwise
	if (typeof window === "undefined") {
		return process.env.INTERNAL_API_URL || "http://localhost:3000"
	}
	// Client-side: use public URL or relative path for embedded API
...
export const auth = {
	async login(credentials: LoginRequest): Promise<ApiResponse<AuthResponse>> {
		return fetchApi<AuthResponse>(`${getBaseUrl()}/auth/login`, {
			method: "POST",
	...
	async register(data: RegisterRequest): Promise<ApiResponse<AuthResponse>> {
		return fetchApi<AuthResponse>(`${getBaseUrl()}/auth/register`, {
			method: "POST",
	...
	async logout(token: string): Promise<ApiResponse<{ message: string }>> {
		return fetchApi<{ message: string }>(`${getBaseUrl()}/auth/logout`, {
			method: "POST",
	...
	async me(token: string): Promise<ApiResponse<{ user: AuthUser }>> {
		return fetchApi<{ user: AuthUser }>(`${getBaseUrl()}/auth/me`, {
			headers: authHeader(token),
		})
	},
}
...
export const calendars = {
	async list(token: string): Promise<ApiResponse<CalendarWithRole[]>> {
		return fetchApi<CalendarWithRole[]>(`${getBaseUrl()}/calendars`, {
			headers: authHeader(token),
		})
	},
	...
	async get(
		...
		return fetchApi<CalendarWithItems>(`${getBaseUrl()}/calendars/${id}`, {
			headers: authHeader(token),
		})
	},
	...
	async update(
		...
		return fetchApi<CalendarWithRole>(`${getBaseUrl()}/calendars/${id}`, {
			method: "PATCH",
			headers: authHeader(token),
			body: JSON.stringify(data),
		})
	...
	async delete(
		...
		return fetchApi<{ success: boolean }>(`${getBaseUrl()}/calendars/${id}`, {
			method: "DELETE",
			headers: authHeader(token),
		})
	},
...
export const items = {
	async create(
		...
		return fetchApi<ItemResponse>(
			`${getBaseUrl()}/calendars/${calendarId}/items`,
			{
				method: "POST",
				headers: authHeader(token),
				body: JSON.stringify(data),
	...
	async update(
		...
		return fetchApi<ItemResponse>(
			`${getBaseUrl()}/calendars/${calendarId}/items/${itemId}`,
			{
				method: "PATCH",
				headers: authHeader(token),
				body: JSON.stringify(data),
	...
	async reorder(
		...
		return fetchApi<{ success: boolean }>(
			`${getBaseUrl()}/calendars/${calendarId}/items/reorder`,
			{
				method: "PATCH",
				headers: authHeader(token),
				body: JSON.stringify(data),
...
export const shares = {
	async list(
		...
		return fetchApi<ShareInfo[]>(
			`${getBaseUrl()}/calendars/${calendarId}/shares`,
			{
				headers: authHeader(token),
			},
		)
	...
	async remove(
		...
		return fetchApi<{ success: boolean }>(
			`${getBaseUrl()}/calendars/${calendarId}/shares/${shareId}`,
			{
				method: "DELETE",
				headers: authHeader(token),
			},
	...
	async generateLink(
		...
		return fetchApi<ShareLink>(
			`${getBaseUrl()}/calendars/${calendarId}/share-link`,
			{
				method: "POST",
				headers: authHeader(token),
			},
	...
	async getByToken(
		...
		return fetchApi<SharedCalendarView>(`${getBaseUrl()}/shared/${shareToken}`)
	},
	...
	async joinByToken(
		...
		return fetchApi<{ calendarId: string }>(
			`${getBaseUrl()}/shared/${shareToken}/join`,
			{
				method: "POST",
				headers: authHeader(token),
			},
...
export const ai = {
	async generateChecklist(
		...
		const url = new URL(`${getBaseUrl()}/ai/calendars/${calendarId}/checklist`)
		if (data) {
			if (data.destination)
	...
	async getRecommendations(
		...
		const url = new URL(
			`${getBaseUrl()}/ai/calendars/${calendarId}/recommendations`,
		)
		url.searchParams.set("destination", data.destination)
